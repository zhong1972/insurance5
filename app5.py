...
# Define the model loading and prediction logic
def load_model():
    model =joblib.load('modelxgb.pkl')  # Provide the correct path to your saved model

    return model    data = request.get_json(force=True)
...
def predict():
    ...
    input_data = np.array(data['features']).reshape(1, -1)  # Adjust shape according to your model's input shape
